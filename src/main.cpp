...
public:
    void createTable(const std::string& cmd) {
        ...
        while (std::getline(ss, col, ',')){
            col = trim(col); if (!col.empty()) columns.push_back(col);
        }
        ...
        for (size_t i = 0; i < columns.size(); ++i){
            file << columns[i]; if (i < columns.size() - 1) file << ",";
        }
    ...
    void insertIntoTable(const std::string& cmd) {
        ...
        for (size_t i = 0; i < values.size(); ++i){ 
            file << values[i]; if (i < values.size() - 1) file << ","; 
        }
    ...
    void updateTable(const std::string& cmd) {
        ...
        if (updatePos == std::string::npos || setPos == std::string::npos){
            std::cout << "Syntax error in UPDATE.\n"; 
            return;
        }   
        ...
        while (std::getline(ss, assignment, ',')) {
            ...
            if (eqPos == std::string::npos) { 
                std::cout << "Syntax error in assignment: " << assignment << "\n"; 
                return; 
            }
        ...
        for (auto& [col, val] : updates) {
            if (headerIndex.find(col) == headerIndex.end()) { 
                std::cout << "Error: Column \"" << col << "\" does not exist.\n"; 
                return; 
            }
        ...
        if (!whereColumn.empty()) {
            if (headerIndex.find(whereColumn) == headerIndex.end()) { 
                std::cout << "Error: Column \"" << whereColumn << "\" does not exist.\n"; 
                return; 
            }
        ...
        for (size_t i = 1; i < rows.size(); ++i) {
            ...
            if (match) { 
                for (size_t j = 0; j < updates.size(); ++j){
                    rows[i][updateIndexes[j]] = updates[j].second; updatedCount++; 
                }
            }
        }
    ...
    void deleteFromTable(const std::string& cmd) { 
        size_t deletePos = cmd.find("DELETE");
        size_t fromPos = cmd.find("FROM");
        size_t wherePos = cmd.find("WHERE");

        if (deletePos == std::string::npos || fromPos == std::string::npos) 
            std::cout << "Syntax error in UPDATE.\n"; return;

        std::string tableName, fromPart, wherePart;

        if (wherePos != std::string::npos) {
            tableName = trim(cmd.substr(deletePos + 6, fromPos - (deletePos + 6)));
            fromPart = trim(cmd.substr(fromPos + 4, wherePos - (fromPos + 4)));
            wherePart = trim(cmd.substr(wherePos + 5));
        } else {
            tableName = trim(cmd.substr(deletePos + 6, fromPos - (deletePos + 6)));
            fromPart = trim(cmd.substr(fromPos + 4));
        }
